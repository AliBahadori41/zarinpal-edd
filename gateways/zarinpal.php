...
class EDD_ZarinPal_Gateway {
	...
	public function receipt( $payment ) {
		...
		if ( $refid ) {
			echo '<tr class="zarinpal-ref-id-row ezp-field"><td><strong>شماره تراکنش بانکی:</strong></td><td>' . $refid . '</td></tr>';
		}
	}
	...
	public function error_reason( $error_id ) {
		...
		switch ( $error_id ) {
			case '-1':
				...
				break;
			case '-2':
				$message = 'IP -2 و يا مرچنت كد پذيرنده صحيح نيست.';
				break;
			case '-3':
			...
			case '-41':
				...
				break;
			case '-42':
				$message = 'مدت زمان معتبر طول عمر شناسه پرداخت بايد بين  30 دقيقه تا 45 روز مي باشد.';
				break;
			case '-54':
